...
window.addEventListener("DOMContentLoaded", function() {
    ...
    e.preventDefault();
    }); //기본동작을 차단 시켜야 이미지를 드래그하고 할 수 있다.(둘다 동작방해 안되게)

    el.addEventListener("drop",function(e) {
    e.preventDefault(); //기본동작을 차단 시켜야 이미지를 드래그하고 할 수 있다. (둘다 동작방해 안되게)

    const files =e.dataTransfer.files;

    const dataset = this.gid;
